...
Select team, 
		Avg(ofga + ofta + oTurnovers) as Pos,
		Avg(ofga + ofta - (dTurnovers + OReb + ODreb)) as RegPos,
		Avg (ofga - otpa) / (ofga + ofta + oTurnovers) as fg2ptpct,
		Avg(otpa / (ofga + ofta + oTurnovers)) as fg3ptpct,
		Avg(oturnovers /(ofga + ofta + oTurnovers)) as turnovrpct,
		Avg(ofta / (ofga + ofta + oTurnovers)) as fouled,
		Avg(oSteals / (dfga + dfta + dTurnovers)) as StealPct,
...
<cfquery datasource="nbastats" name="GetAvgs">
Select team, 
		Avg(dfga + dfta + dTurnovers) as Pos,
		Avg(dfga + dfta - (oTurnovers + dReb + dDreb)) as RegPos,
		Avg (dfga - dtpa) / (dfga + dfta + dTurnovers) as fg2ptpct,
		Avg(dtpa / (dfga + dfta + dTurnovers)) as fg3ptpct,
		Avg(dturnovers /(dfga + dfta + dTurnovers)) as turnovrpct,
		Avg(dfta / (dfga + dfta + dTurnovers)) as fouled,
		Avg(dSteals / (ofga + ofta + oTurnovers)) as StealPct,
