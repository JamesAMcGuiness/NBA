...
<cfquery datasource="Nba" name="GetStatus">
	Select * from NBASTATUS	
</cfquery>

<cfif GetStatus.Status neq 'RUNGAMESIM'>
	<cfabort showerror="NBASTATUS table not set to RUNGAMESIM">
</cfif>
 --->
...
<cfoutput>runct is #myrunct#</cfoutput>
...
<cfoutput query="GetSpds">
	...
	Select f.LastSeven as fhlth,u.LastSeven as uhlth
	from Teamhealth f, TeamHealth u
	where f.Team = '#fav#' 
	and u.team = '#und#' 
	</cfquery>

	<cfset hadvfor = ''>
	<cfif GetHealth.recordcount is 0>
		<cfset hlthdif = 0>
	
	<cfelse>
	
		
		<cfif GetHealth.fhlth gt GetHealth.uhlth >
			<cfset hlthdif = GetHealth.fhlth - GetHealth.uhlth>
			<cfset hadvfor = 'FAV'>
		<cfelse>
			<cfset hlthdif = GetHealth.uhlth - GetHealth.fhlth>
			<cfset hadvfor = 'UND'>
		</cfif>
	</cfif>
	...
	<cfif hadvfor is 'FAV'>
		<cfset FavAddPts = AddPts>
	</cfif>

	<cfif hadvfor is 'UND'>
		<cfset UndAddPts = AddPts>
	</cfif>


	#fav#/#und#....The health dif is #hlthdif#....Adv for #hadvfor#...The FAV Addpts is #favaddpts#, the UND Addpts is #UndAddPts#
...
<cfif #UseOverFGA# is true>
	...
	<cfquery datasource="nba" name="GetData" cachedWithin="#CreateTimeSpan(0,2,0,0)#">
		Select 
			ooFGAZ1                     as z1,
...
-----------------------------------------------<br>
 --->
	...
	<cfset skipit = false>
	<cfif favsc lt 70 or favsc gt 130>
		<cfset skipit = true>
	</cfif>
	
	<cfif undsc lt 70 or undsc gt 130>
		<cfset skipit = true>
	</cfif>
	...
	<cfif Skipit is false>
		...
		<cfif Ha is 'H' >
			<cfset FavPredScore[gg] = FavPredScore[gg] + testval + FavAddpts>
		<cfelse>
			<cfset UndPredScore[gg] = UndPredScore[gg] + testval + UndAddpts>
		</cfif>
...
<cfif goodct neq 0>	
	<cfset spddif = totFav/goodct - totUnd/goodct>

	****************************************************<br><br>
	#fav#: #totFav/goodct#<br>
	#und#: #totUnd/goodct#<br>
	Our Spread: #spddif#<br>
	Vegas Spread: #spd#<br>
	Our Total Points: #(totFav/goodct) + (totUnd/goodct)#<br>
	Vegas Total: #myou# <br> 
	****************************************************<br><br>
	...
	<cfif (totFav/goodct - totUnd/goodct) gt val(spd)>
	
		<cfif (totFav/goodct - totUnd/goodct) gt val(spd) + 8>
			<cfset spdbigcov = 1>
		</cfif>
		
	</cfif>

	<cfif (totFav/goodct - totUnd/goodct) le val(spd)>
	
		<cfif (totFav/goodct - totUnd/goodct) lt val(spd) - 8>
			<cfset spdbigcov = 1>
		</cfif>
	...
	<cfif (totFav/goodct + totUnd/goodct) gt val(myou)>
	
		<cfif (totFav/goodct + totUnd/goodct) gt val(myou) + 8>
			<cfset totbigcov = 1>
		</cfif>
	</cfif>


	<cfif (totFav/goodct + totUnd/goodct) le val(myou)>
	
		<cfif (totFav/goodct + totUnd/goodct) lt val(myou) - 8>
			<cfset totbigcov = 1>
		</cfif>
	</cfif>


	<cfset totpoints = totFav/goodct + totUnd/goodct>
...
<cfif goodct neq 0 and (totFav/goodct) neq 0 >

<!-- Loop through scores and create deltas -->
<cfloop index="ii" from="1" to="#goodct#">
	<cfloop index="jj" from="1" to="#goodct#">
...
<cfloop index="ii" from="1" to="#Arraylen(mydelta)/2#">
	<cfloop index="jj" from="1" to="#Arraylen(mydelta)/2#">
		...
		<cfif mydelta[ii][jj] gt spd>
			...
			<cfif mydelta[ii][jj] gt spd + 8>
				<cfset BigFavCovCt = BigFavCovCt + 1>
			</cfif>
		<cfelse>
			<cfif mydelta[ii][jj] lt spd>	
				<cfset UndCovCt = UndCovCt + 1>

				<cfif mydelta[ii][jj] + 8 lt spd>
					<cfset BigUndCovCt = BigUndCovCt + 1>
				</cfif>
			<cfelse>
...
<cfif FavCovct gt UndCovct>
	...
	<cfset BigFavCovRat = ((BigFavCovCt/Gamect)*100)>
	
<cfelse>
	<cfset pick = '#Und#'>
	<cfset Rating = ((UndCovct/GameCt)*100)>
	<cfset BigUndCovRat = ((BigUndCovCt/Gamect)*100)>
</cfif>


<cfset oupick = 'U'>
<cfset ourating = 100 - Numberformat((Overct/(goodct*goodct))*100,'999.99')>

<cfif (Overct/(goodct*goodct))*100 gt 50>
	<cfset oupick = 'O'>
	<cfset ourating = #Numberformat((Overct/(goodct*goodct))*100,'999.99')#>
</cfif>
...
<cfif Pick is '#fav#'>
<!--- 	FAVORITE #Fav# covered #Numberformat((FavCovct/GameCt)*100,'999.99')#<br>
	Big Cover Rate #Numberformat((BigFavCovct/GameCt)*100,'999.99')#<br> --->
	<cfset BOSideRat = #Numberformat((BigFavCovct/GameCt)*100,'999.99')#>
</cfif>
<cfif Pick is '#und#'>
<!--- 	UNDERDOG #Und# covered #Numberformat((UndCovct/GameCt)*100,'999.99')#<br>
	Big Cover Rate #Numberformat((BigUndCovct/GameCt)*100,'999.99')#<br> --->
	<cfset BOSideRat = #Numberformat((BigUndCovct/GameCt)*100,'999.99')#>
</cfif>
<cfif oupick is 'O'>
<!--- Over Win Rate was #ourating#<br>
#myou# Strong Over Win Rate was #Numberformat((hict/(goodct*goodct))*100,'999.99')#<br> --->
	<cfset BOTotalRat = #Numberformat((hict/(goodct*goodct))*100,'999.99')#>
<cfelse>
<!--- Under Win Rate was #ourating#<br>
#myou# Strong Under Win Rate was #Numberformat((loct/(goodct*goodct))*100,'999.99')#<br> --->
	<cfset BOTotalRat = #Numberformat((loct/(goodct*goodct))*100,'999.99')#>
</cfif>
...
<cfquery datasource="nba" name="gs">
...
<cfquery datasource="nba" name="gsf">
Select gsf.Fav, Avg(gsf.Rating) as aRating, count(gsf.pick) as Wins, Avg(gsf.BOSideRat) as BoSide
from nbagamesim gsf
...
<cfquery datasource="nba" name="gsu">
Select gsu.Und, Avg(gsu.Rating) as aRating, count(gsu.pick) as Wins, Avg(gsu.BOSideRat) as BoSide
from nbagamesim gsu
Where GameTime = '#Session.Gametime#'
...
<cfquery datasource="nba" name="gsov">
Select Avg(gsov.ouRating) as aRating, count(gsov.oupick) as Wins, Avg(gsov.BOTotalRat) as BoTotal
...
<cfquery datasource="nba" name="gsun">
Select Avg(gsun.ouRating) as aRating, count(gsun.oupick) as Wins, Avg(gsun.BOTotalRat) as BoTotal
...
------------------------------------------------------------------------<br>
FAV(#gsf.Fav#) Wins:#gsf.wins# and rating of: #Numberformat(gsf.aRating,'999.99')#<br>
Big FAV Cover Rate: #Numberformat(gsf.BoSide,'999.99')#<br><br>
UND(#gsu.Und#) Wins:#gsu.wins# and rating of: #Numberformat(gsu.aRating,'999.99')#<br>
Big UND Cover Rate: #Numberformat(gsu.BoSide,'999.99')#<br><br>
Over Wins:#gsov.wins# and rating of: #Numberformat(gsov.aRating,'999.99')#<br>
Big OVER Cover Rate: #Numberformat(gsov.BoTotal,'999.99')#<br><br>
Under Wins:#gsun.wins# and rating of: #Numberformat(gsun.aRating,'999.99')#<br>
Big UNDER Cover Rate: #Numberformat(gsun.BoTotal,'999.99')#<br>
------------------------------------------------------------------------<br>
<br>
</cfoutput>
...
<cfcatch type="any">
	...
	<cfhttp method='post'
		...
		<cfhttpparam name='mySubject' type='FormField' value="Error GameSimUltimateTest2007.cfm">
