...
<cfquery  datasource="nba" name="GetAdv">
	Select 
	  Team,	
	  Avg(oassists) as oass,
	  Avg(dassists) as dass,
	  Avg(oPIP) as ooPIP,
	  Avg(dPIP) as ddPIP,
...
<cfloop query="getteams">
	...
	Select 
	  ...
	  f.oassists - a.dassists as oPowass,
	  a.oassists - f.dassists as dPowass,	  
	  	  
	  f.oPIP - a.dPIP as oPowPIP,
	  a.oPIP - f.dPIP as dPowPIP,	  	  
	...
	and a.Team in (Select opp from nbadata where team = '#fav#' and gametime < '#mygametime#')
	and f.mins = 240 
	order by 8
	</cfquery>
	...
	<cfif oPowAss Gt 0>
		<cfset oPowAssistsover = oPowAssistsover + 1>
	</cfif>
	
	<cfif dPowAss Gt 0>
		<cfset dPowAssistsover = dPowAssistsover + 1>
	</cfif>


	<cfif oPowPIP Gt 0>
		<cfset oPowPIPover = oPowPIPover + 1>
	</cfif>
	
	<cfif dPowPIP Gt 0>
		<cfset dPowPIPover = dPowPIPover + 1>
	</cfif>
	...
	#100*(oPowPIPover/ct)#,
	#100*(dPowPIPover/ct)#,
	#100*(oPowAssistsover/ct)#,
	#100*(dPowAssistsover/ct)#
...
Defense - FGpct	
	...
	<!--- <cfif dPowFGpctover neq 0 and dPowFGpctunder neq 0>	
	<br>dFGPCT....<br>
	#fav#<br>
...
<cfcatch type="any">
	...
	<cfhttp method='post'
		...
		<cfhttpparam name='mySubject' type='FormField' value="Error:CreateAvgs.cfm">
