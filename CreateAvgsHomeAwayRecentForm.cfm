...
<cfset myDateCheck = Replace(Mid(ToString(DateAdd("d",myDate,-360)),6,10),'-','','all')>

<!--- Create the Avgs for each team --->
<cfquery  datasource="nbastats" name="GetAdvHome">
	Select 
	  Team,	
 	  Avg(ofga - otpa) as ohfga,
	  Avg(otpa)        as ohTpa,	
      Avg(oFta)        as ohFta,
	   
	  Avg ( (ofgm - otpm) / (ofga - otpa)) as ohfgpct,
	  Avg  (otpm / otpa)                as ohTppct, 
	  Avg (oftm / ofta)                   as ohFTpct,  
	   
	  Avg(dfga - dtpa) as dhfga,
	  Avg(dtpa)        as dhTpa,	
      Avg(dFta)        as dhFta,
	   
	  Avg ( (dfgm - dtpm) / (dfga - dtpa) ) as dhfgpct,
	  Avg  (dtpm / dtpa)                as dhTppct,
	  Avg (dftm / dfta)                   as dhFTpct  
...
<cfquery  datasource="nbastats" name="GetAdvAway">
	Select 
	  Team,	
 	  Avg(ofga - otpa) as oafga,
	  Avg(otpa)        as oaTpa,	
      Avg(oFta)        as oaFta,
	   
	  Avg ( (ofgm - otpm) / (ofga - otpa)) as oafgpct,
	  Avg  (otpm / otpa)                as oaTppct, 
	  Avg (oftm / ofta)                   as oaFTpct,  
	   
	  Avg(dfga - dtpa) as dafga,
	  Avg(dtpa)        as daTpa,	
      Avg(dFta)        as daFta,
	   
	  Avg ( (dfgm - dtpm) / (dfga - dtpa) ) as dafgpct,
	  Avg  (dtpm / dtpa)                as daTppct,
	  Avg (dftm / dfta)                   as daFTpct  
